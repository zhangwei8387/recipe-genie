...
- 主要功能（MVP 范围）：
  ...
  - Mock AI 服务：在初期使用规则/映射的本地 Mock 实现以快速验证 UX，后期可替换为真实模型（OpenAI 或内部模型）。
...
- 技术建议（参考实现）：
  - 前端：React + Vite；组件：`RecipeView`、`IngredientList`、`InstructionList`、`NutritionSummary`、`ChatPanel`。
  - 后端：FastAPI 或 Express（轻量）；API：GET `/api/recipes/{id}`、POST `/api/recipes/{id}/transform`。
  - 数据：Recipe JSON 模型（id, title, ingredients[], steps[], nutrition{}）；临时存储可用文件或内存实现。
  - AI：初期使用 Mock 规则引擎（基于小型映射表与转换函数），后期接入真实模型并保留 prompt/response 审计。

- 交付物与里程碑（建议）：
  - M1（2 周）: 前端静态菜谱视图 + Mock Transform API + ChatPanel 基本联动；包含至少 2 个示例指令的演示。  
  - M2（4 周）: 增加变更历史保存、测试用例、并完成 UX 微调与人工审核流程（prompt 保存与审查注记）。  
  - M3（6 周）: 可选：替换 Mock AI 为真实模型并补充性能/安全评估。
